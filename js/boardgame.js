(function() {
	HTMLCanvasElement.prototype.toBoard = function(config) {
		...
		board.init(config);
		board.render();
		self.onCellClick = self.onCellClick || function(callback) {
			board.onCellClick(callback);
		};
		self.drawPiece = self.drawPiece || function(cell, piece) {
...
Cell.prototype.equals = function(cell) {
	if (cell && this.column === cell.column 
		&& this.row === cell.row)
		return true;
	return false;
};
...
var Board = {
	init: function(config) {
		...
		self.width = config.width === 'auto' 
					? browserDimension[0] : config.width;
		self.height = config.height === 'auto' 
					? browserDimension[1] : config.height;
	...
	render: function() {
		...
		if (self.cellColor)
			self.setCellColors(self.cellColor, self.alternateCellColor);
		self.writeCellIds();
		if (self.pieces)
			self.drawPieces();
		self.bindEvents();
	...
	createCells: function() {
		...
		for (var i = 0; i < self.columnLength; i++) {
			for (var j = 0; j < self.rowLength; j++) {
				...
				if (self.useAlgebraicNotation)
					index = alphabetArray[i] + (self.rowLength - j);
				else if (self.nonZeroBasedIndex)
					index += 1;
	...
	setCellColors: function(primaryColor, alternateColor) {
		...
		for (var i = 0; i < self.cells.length; i++) {
			...
			x = coords[0];
			y = coords[1];
			if (even && ((parseInt(i / self.rowLength) + i) % 2 == 0) || 
				(!even && i % 2 == 0)) {
	...
	hasPieceOnId: function(id) {
		var self = this;
		if (id) {
			if (self.cellPieces.indexOf(id) > -1)
				return true;
		}
		return false;
	},
	...
	getCellById: function(id) {
		var self = this;
		if (self.useAlgebraicNotation)
			return self.cells[self.cellIds.indexOf(id)];
